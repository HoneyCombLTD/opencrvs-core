...
const getApplicationConfig = () => {
  return request<IApplicationConfigResponse>({
    url: new URL('/publicConfig', process.env.CONFIG_API_URL).toString(),
    method: 'GET'
  })
}
...
const authenticate = (data: IAuthenticationData) => {
  return request<IAuthenticateResponse>({
    url: new URL('authenticate', process.env.AUTH_API_URL).toString(),
    method: 'POST',
    data
  })
}
...
const verifyCode = (data: ICodeVerifyData): Promise<IAuthenticateResponse> => {
  return request({
    url: new URL('verifyCode', process.env.AUTH_API_URL).toString(),
    method: 'POST',
    data
  })
}
...
const verifyUser = (
  ...
  return request({
    url: new URL('verifyUser', process.env.AUTH_API_URL).toString(),
    method: 'POST',
...
const verifyNumber = (
  ...
  return request({
    url: new URL('verifyNumber', process.env.AUTH_API_URL).toString(),
    method: 'POST',
    data: { nonce, code }
  })
}
...
const verifySecurityAnswer = (
  ...
  return request({
    url: new URL('verifySecurityAnswer', process.env.AUTH_API_URL).toString(),
    method: 'POST',
    data: { nonce, answer }
  })
}
...
const changePassword = (nonce: string, newPassword: string): Promise<void> => {
  return request({
    url: new URL('changePassword', process.env.AUTH_API_URL).toString(),
    method: 'POST',
...
const sendUserName = (nonce: string): Promise<void> => {
  return request({
    url: new URL('sendUserName', process.env.AUTH_API_URL).toString(),
    method: 'POST',
    data: { nonce }
  })
}
